	...
	<form @submit.prevent="handleSubmit" class="space-y-stack-md">
		...
		<div class="space-y-1">
			...
			<p class="font-body-base text-on-surface-variant">
				Sign in to your BinarySync workspace.
			</p>
		</div>
		...
		<div class="space-y-1">
			<div class="flex justify-between items-center">
				...
				<a
					href="#"
					class="font-label-caps text-label-caps text-primary hover:underline uppercase"
				>
					Forgot?
				</a>
			</div>
		...
		<div class="flex items-center gap-2">
			...
			<label
				for="remember"
				class="font-body-sm text-on-surface-variant cursor-pointer select-none"
		...
		<button
			type="submit"
			:disabled="loading"
			class="w-full bg-primary text-on-primary font-semibold py-3.5 rounded-xl shadow-md hover:opacity-90 active:scale-[0.98] transition-all flex justify-center items-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
		>
			<span
				v-if="loading"
		...
		<div class="grid grid-cols-2 gap-3">
			<button
				...
				@click="handleGoogle"
				:disabled="loading"
				class="flex items-center justify-center gap-2 border border-outline-variant rounded-lg py-3 bg-white hover:bg-surface-container-low transition-colors font-body-sm font-semibold disabled:opacity-60"
			>
...
async function handleSubmit() {
	...
	try {
		...
		// Error notification already shown by the store
	} finally {
		loading.value = false;
	}
}
